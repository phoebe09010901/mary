...
//album3
$query = "select Fullkey, title from $table_name_album where category=4 and pub=1 order by album_date desc, Fullkey desc";
				...
				for($i=0; $i<$page_num; $i++) {
                    ...
                    <div class="photo_block_3">
                        <div style="width:170px; height:120px; background:#D5E044; color:#000; overflow:hidden;">
                            <a href="happytails_4.php?album_id=<?=$album['Fullkey']?>"><img class="example-image" src="<?='album_photos/'.$album['Fullkey'].'/'.$photo['file1']?>" <?=$img_str?> border="0" alt="<?=$album['title']?>" title="<?=$album['title']?>"/></a>
                        </div><?=$album['title']?>
                    </div>
                    <?php
					}
