...
$dog_id1   = format_data($_GET['dog_id1'], 'int');
$dog_id2   = format_data($_GET['dog_id2'], 'int');
$dog_id3   = format_data($_GET['dog_id3'], 'int');
$file_num  = 1;
$_width    = 800;
$_height   = 600;
...
if($_POST['action']=='save') {
	...
	$dog_id2 = format_data($_POST['dog_id2'], 'int');
	$dog_id3 = format_data($_POST['dog_id3'], 'int');
	$key_c   = format_data($_POST['key_c'], 'text');
	$key_c_c = format_data($_POST['key_c_c'], 'text');
	$adopion_session_id = format_data($_POST['adopion_session_id'], 'text');
    //check check_code
	...
	//apply1
	$query = "insert into ".$table_name_dogs."_apply1(dog_id, dog_id2, dog_id3";
	for($i=1; $i<=$apply1_num; $i++) {
		$query .= ", ans".$i."";
	}
	$query .= ", pub, create_time) values('$dog_id', '$dog_id2', '$dog_id3'";
	for($i=1; $i<=15; $i++) {
		$query .= ", '".$_POST['ans'.$i]."'";
	}
	...
	$array_files = glob('doggie/'.$adopion_session_id.'/*');

	if(count($array_files)>0) {
		foreach($array_files as $key => $value) {
			$file_name = basename($value);
			if(is_file('doggie/'.$adopion_session_id.'/'.$file_name)) {
				$obj_image->copy_file('doggie/'.$adopion_session_id.'/'.$file_name, 'doggie/'.$dog_id.'/'.$apply_id.'/'.$file_name);
				$obj_image->del_file('doggie/'.$adopion_session_id.'/'.$file_name);
			}
		}
	}
	rmdir('doggie/'.$adopion_session_id);
	//send mail
	$obj_mail->apply($apply_id);
	/*echo '<script type="text/javascript" src="js/jquery-1.11.0.min.js"></script><script type="text/javascript" src="js/jquery.fancybox.js?v=2.1.5"></script>';
	js_a_l('We have received your information will contact you as soon as possible, thank you.', 'fancyboxend');exit;*/
	js_a_l('We have received your information will contact you as soon as possible, thank you.', 'index.php');exit;
}
if(!$dog_id1 && !$dog_id2 && !$dog_id3) {
	js_a_l('Please choice a dog at least.', 'back');exit;
}
//dogs
$query = "select * from $table_name_dogs where Fullkey='$dog_id'";
$dogs = $obj_dogs->run_mysql_out($query);
//dogs
$query = "select * from $table_name_dogs where Fullkey='$dog_id1'";
$dogs1 = $obj_dogs->run_mysql_out($query);
//dogs
$query = "select * from $table_name_dogs where Fullkey='$dog_id2'";
$dogs2 = $obj_dogs->run_mysql_out($query);
//dogs
$query = "select * from $table_name_dogs where Fullkey='$dog_id3'";
$dogs3 = $obj_dogs->run_mysql_out($query);
...
function real_save() {
	...
	//}else if(!$("#ans17").val()) {
        ...
        $("#ans19").focus();
		check++;
	//}else if(!$("#ans26").val() && !$("#ans27").val() && !$("#ans28").val()) {
		//alert("Please enter Home Tel.");
		//check++;
	}else if(!$("#ans29").val() && !$("#ans30").val() && !$("#ans31").val()) {
...
function show_list() {
	$("#photos_list").html('');
	var ajaxurl = 'doggie_data.php?action=photos_list&dog_id=<?=$dog_id1?>';
	$.ajax({
		url: ajaxurl,
		dataType: 'json',
		success: function(request) {
			$.each(request, function(key, photo_data) {
				$.each(photo_data, function(key, data)	{
					var div_str = '<div id="show_photo_block_'+key+'" class="show_photo" onmouseover="mouseover_show_btn(\''+key+'\')" onmouseout="mouseover_hidden_btn(\''+key+'\')"><div class="photo_block"><div id="photo_del_'+key+'" class="del_btn" onclick="delete_photo(\''+key+'\', \''+data['file1']+'\')"><img src="images/close.png" border="0"></div><img src="doggie/<?=$dog_id?>/<?=$my_session_id?>/'+data['file1']+'" width="'+data['width']+'" height="'+data['height']+'" alt="'+data['title']+'" id="photo_file1_'+key+'"></div></div>';//alert(div_str);
...
function delete_photo(photo_id, file1) {
	if(confirm("確定要刪除 ("+file1+") 此張相片??")) {
		var ajaxurl = 'doggie_data.php?action=delete_photo&dog_id=<?=$dog_id1?>&file1='+file1;
		$.ajax({
			url: ajaxurl,
			dataType: 'html',
			success: function(request) {
...
$(document).ready(function () {
	...
	$("#file1_upload").uploadify({
			...
			formData   : { "action": "upload_photo", "main_str": "dogs", "dog_id": "<?=$dog_id1?>", "my_session_id": "<?=$my_session_id?>"},
...
<form method="post" action="adoption_apply_01.php" name="apply_form" id="apply_form">
<input type="hidden" name="action" value="save" />
<input type="hidden" name="adopion_session_id" value="<?=$my_session_id?>" />
<input type="hidden" name="dog_id" value="<?=$dog_id1?>" />
<input type="hidden" name="dog_id2" value="<?=$dog_id2?>" />
<input type="hidden" name="dog_id3" value="<?=$dog_id3?>" />
                            	...
                            	<td>
                                	<table width="95%" border="0" cellspacing="0" cellpadding="3">
                                        ...
                                        <tr>
                                            ...
                                            <td><input class="text_line" type="text" name="ans1" value="<?=stripslashes($dogs1['name'])?>" style="width:100%" readonly /></td>
                                        </tr>
                                        <tr>
                                        	<td><span class="re_main4">(2nd Choice)</span></td>
                                            <td><input class="text_line" type="text" name="ans2" value="<?=stripslashes($dogs2['name'])?>" style="width:100%" readonly /></td>
                                        </tr>
                                        <tr>
                                        	<td><span class="re_main4">(3rd Choice)</span></td>
                                            <td><input class="text_line" type="text" name="ans3" value="<?=stripslashes($dogs3['name'])?>" style="width:100%" readonly /></td>
                                        </tr>
                                    </table>
                                </td>
                            </tr>
	            ...
	            <tr>
	            	<td>
                    	<table width="100%" border="0" cellspacing="0" cellpadding="0">
                                        	...
                                        	<td>
                                            	<table width="0" border="0" cellpadding="3" cellspacing="0">
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td valign="top"><img src="tools/chucksum.php?key_c=<?=$key_c_c?>" alt="Change another verification code" id="rand-img" width="120" height="30" name="chucksumImg" border="1" style="cursor:hand" >
                                                        <input name="key_c" type="text" class="css-checkbox" id="key_c" value="" /><input name="key_c_c" type="hidden" id="key_c_c" value="<?=$key_c_c?>" /></td>
                                                    </tr>
                                                </table>
                                            </td>
                                        </tr>
                            ...
                            <tr>
                            	<td height="150" align="center" valign="middle"><!--<span class="re_main4_btn"><a href="#" onClick="javascript:print();">Print</a></span> --><span class="re_main4_btn" style="margin-right:10px;"><a onClick="#" style="cursor:pointer" href="wistlist_01.php">Go Back</a></span><span class="re_main4_btn"><a onClick="real_save()" style="cursor:pointer">Send</a></span><!--&nbsp;&nbsp;<span class="re_main4_btn"><a href="?save=1">Save to Html</a></span><br><br><br>How to save the PDF file：<span class="re_main6_btn"><a href="download/savetopdf_PC.pdf" target="new" style="width: 200px;">PC</a></span>&nbsp;、&nbsp;<span class="re_main6_btn"><a href="download/savetopdf_MAC.pdf" target="new" style="width: 200px;">MAC</a></span>--></td>
                            </tr>
                            <tr>
                            	<td height="1" bgcolor="#C2CB56"></td>
                            </tr>
