...
if($_POST['action']=='save') {
	...
	$name    = format_data($_POST['name'], 'content');
	...
	if(!$Fullkey)
		$query = "insert into $table_name(lang, name, content, prev, lv, sort, pub, create_time) values('".$lang."', '".$name."', '".$content."', '".$prev."', '".$lv."', '".$sort."', 1, now())";
	elseif($Fullkey)
		$query = "update $table_name set name='".$name."', content='".$content."', prev='".$prev."', lv='".$lv."', sort='".$sort."', edit_time=now() where Fullkey='".$Fullkey."'";
   		...
   		<form method="post" id="<?=$main_str?>_category_form" action="<?=$_SERVER['PHP_SELF']?>">
            ...
            <table class="<?=$main_str?>-table">
                ...
                <tr>
                    ...
                    <td align="left"><?php $sprods->data_category_select_reloop('news_category', 0, $prev, News_Category_Lv_Num); ?></td>
                </tr>
                <?php } ?>
                <tr>
				...
				?>
                <tr>
                    <td width="120" height="60">內容敘述：</td>
                    <td align="left"><textarea id="content" name="content" rows="10" style="width:400px"><?=stripslashes($cate['content'])?></textarea></td>
                </tr>
                <?php
				}
				?>
