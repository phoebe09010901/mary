...
if($_POST['action']=='save') {
	...
	if(!$Fullkey)
		$query = "insert into ".$table_name."(lang, category, file1, file2, file3, youtube1, youtube2, youtube3, youtube4, youtube5, youtube6, youtube7, youtube8, youseq1, youseq2, youseq3, youseq4, youseq5, youseq6, youseq7, youseq8, name, sex, breed, color, neuter, microchip, rescuer, years, month, weight, remark, handler, sort, verify, adopter, pub, hit_counts, create_time, edit_time) values('".$lang."', '".$category."', '".$file1."', '".$file2."', '".$file3."', '".$youtube1."', '".$youtube2."', '".$youtube3."', '".$youtube4."', '".$youtube5."', '".$youtube6."', '".$youtube7."', '".$youtube8."', '".$youseq1."', '".$youseq2."', '".$youseq3."', '".$youseq4."', '".$youseq5."', '".$youseq6."', '".$youseq7."', '".$youseq8."', '".$name."', '".$sex."', '".$breed."', '".$color."', '".$neuter."', '".$microchip."', '".$rescuer."', '".$years."', '".$month."', '".$weight."', '".$remark."', '".$handler."', '".$sort."', '".$verify."', '".$adopter."', 1, 0, now(), now())";
	elseif($Fullkey)
		$query = "update ".$table_name." set category='".$category."', file1='".$file1."', file2='".$file2."', file3='".$file3."', youtube1='".$youtube1."', youtube2='".$youtube2."', youtube3='".$youtube3."', youtube4='".$youtube4."', youtube5='".$youtube5."', youtube6='".$youtube6."', youtube7='".$youtube7."', youtube8='".$youtube8."', youseq1='".$youseq1."', youseq2='".$youseq2."', youseq3='".$youseq3."', youseq4='".$youseq4."', youseq5='".$youseq5."', youseq6='".$youseq6."', youseq7='".$youseq7."', youseq8='".$youseq8."', name='".$name."', sex='".$sex."', breed='".$breed."', color='".$color."', neuter='".$neuter."', microchip='".$microchip."', rescuer='".$rescuer."', years='".$years."', month='".$month."', weight='".$weight."', handler='".$handler."', sort='".$sort."', verify='".$verify."', adopter='".$adopter."', edit_time=now() where Fullkey='".$Fullkey."'";
	$obj_dogs->run_mysql($query);

	if(!$Fullkey) {
		$dog_id = mysql_insert_id();
		$query = "insert into ".$table_name."2(dog_id, content, content2) values('".$dog_id."', '".$content."', '".$content2."')";
		$obj_dogs->run_mysql($query);
    ...
    $Fullkey = $dog_id;
		...
		$query = "update ".$table_name."2 set content='".$content."', content2='".$content2."' where dog_id='".$Fullkey."'";
		$obj_dogs->run_mysql($query);
	}
	...
	$query   = "select * from ".$table_name."2 where dog_id='".$Fullkey."'";
	...
	$dogs2['content'] = '<table align="center" border="0" cellpadding="0" cellspacing="0"
height="191" width="95%">
<tbody>
<tr>
<td class="pro_main_s">
...
20.79px;">&#39;Agreement&#39;.</span>
	...
	$dogs2['content2'] = '<table border="0" cellpadding="0" cellspacing="0" width="100%">
<tbody>
<tr>
   		...
   		<form method="post" id="<?=$main_str?>_form" action="<?=$_SERVER['PHP_SELF']?>">
            ...
            <table class="<?=$main_str?>-table">
              <tbody>
                ...
                <?php } ?>
                <tr>
                    <td colspan="4" height="30" style="background-color: #FF9900; color: white;">Youtube管理（排序說明：數字越大越前面）</td>
                </tr>
                    ...
                    for($i=1; $i<=8; $i++){                        
                        if($Fullkey){
                            if($dogs['youseq'.$i] == 0){
                                ...
                                $youseq = $dogs['youseq'.$i];
                            }

                            if($dogs['youtube'.$i] != ""){
                                $youArr[$youseq] = $dogs['youtube'.$i];
                            }
                        } else {
                            $youArr[$i*10] ='#';
                        }
                ...
                ?>
                    <td width="120" height="60">Youtube<?=$i?>：</td>
                    <td align="left">
                        <input type="text" name="youtube<?=$i?>" id="youtube<?=$i?>" value="<?=$value?>" class="frome" style="width:250px" />
                        <input type="text" name="youseq<?=$i?>" id="youseq<?=$i?>" value="<?=$key?>" class="frome" style="width:50px" />
                    </td>
                <?
                    if($i%2 == 0) {
                ?>
                ...
                ?>
                    <td width="120" height="60">Youtube<?=$i?>：</td>
                    <td align="left">
                        <input type="text" name="youtube<?=$i?>" id="youtube<?=$i?>" value="" class="frome" style="width:250px" />
                        <input type="text" name="youseq<?=$i?>" id="youseq<?=$i?>" value="" class="frome" style="width:50px" />
                    </td>
                <?
                    if($i%2 == 0) {
                ?>
                ...
                <tr>
                    ...
                    <td align="left" colspan="4">
											<input type="checkbox" name="adopted" id="adopted" value="1" <?php if($agree['adopted']==1){echo 'checked';} ?> disabled>Adopted　
											<input type="text" name="adopter" id="adopter" value="<?=($dogs['adopter'])?$dogs['adopter']:$_SESSION[Login_System_User]['id']?>" class="frome" style="width:250px"> <a href="dogs_agreement_preview.php?agreement_id=<?=$agree['Fullkey']?>" target="_blank">Agreement Link</a><br />
											<textarea name="adopte_info" id="adopte_info" class="frome" style="width:100%; height:75px" readonly><?=$agree['adopte_info']?></textarea>
										</td>
                </tr>
                <tr>
                    <td height="60">說明文一：</td>
                    <td align="left" colspan="4"><textarea id="content" name="content" rows="10"><?=stripslashes($dogs2['content'])?></textarea></td>
                </tr>
                <tr>
                    <td height="60">說明文二：</td>
                    <td align="left" colspan="4"><textarea id="content2" name="content2" rows="10"><?=stripslashes($dogs2['content2'])?></textarea></td>
                </tr>
                <tr>
