...
$query = "select Fullkey, name, breed, neuter, file1, sex, years, month, weight from ".$table_name_dogs." where boarding=1 and boarding_state=0 $where_str order by name asc";
			...
			?>
                    <div class="<?=($i%5==0)?'boardingList_adoption_2_all_first':'boardingList_adoption_2_all'?>">
                        ...
                        <div class="boardingList_adoption_2_2">
                            ...
                            <span class="center1"><?=stripslashes($dogs['breed'])?><br /> <?=$neuter?> <?=$dogs['sex']?><br /> <?=($dogs['years']>0)?stripslashes($dogs['years']).' year(s)':''?><?=($dogs['month'])?' '.$dogs['month'].' month(s)':''?>, <?=stripslashes($dogs['weight'])?> lbs.</span>
                        </div>
                    </div>
            <?php	
			}
