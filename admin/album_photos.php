...
if ($_GET['album_id']) {
	...
	$query = "select Fullkey, category, title, album_date from $table_name where Fullkey='".$album_id."'";
...
function show_list() {
	$("#photos_list").html('');
	var ajaxurl = '<?=$main_str?>_data.php?action=photos_list&album_id=<?=$album['Fullkey']?>';
	$.ajax({
		url: ajaxurl,
		dataType: 'json',
...
function keydown_text(photo_id, update_row, e) {
	if(e.keyCode==13) {
		var ajaxurl = '<?=$main_str?>_data.php?action=update_photo&album_id=<?=$album['Fullkey']?>&photo_id='+photo_id+'&update_row='+update_row+'&update_text='+encodeURIComponent($("#photo_"+update_row+"_"+photo_id).val());
		$.ajax({
			url: ajaxurl,
			dataType: 'html',
			success: function(request) {
...
function delete_photo(photo_id, file1, title) {
	...
	if(confirm("確定要刪除 "+title+"("+file1+") 此張相片??")) {
		var ajaxurl = '<?=$main_str?>_data.php?action=delete_photo&album_id=<?=$album_id?>&photo_id='+photo_id+'&file1='+file1;
		$.ajax({
			url: ajaxurl,
			dataType: 'html',
			success: function(request) {
...
$(document).ready(function () {
	...
	for($i=1; $i<=$file_num; $i++) { 
		$obj_image->uploadify_js('file'.$i.'_upload', 'file'.$i, 'show_file'.$i, array("action", "main_str", "album_id"), array("upload_photo", $main_str, $album['Fullkey']));
	} 
	?>
	show_list();
});
...
<div class="admin-panel">
  <?php include("include_menu.php"); ?><!--slidebar end-->
   ...
   <ul class="topbar">
      ...
      <a href="album.php?category=<?=$album['category']?>"><li class="left"><?=$album['album_date'].' '.$album['title']?></li></a>
