...
$search_type= (!$_GET['search_type'])?'output0':format_data($_GET['search_type'], 'text');
...
if($_GET['action']=='delete') {
	...
	js_a_l('刪除成功', 'back');exit;
}elseif($_POST['action']=='output_all') {	//批次修改狀態
	$my_query_string = format_data($_POST['my_query_string'], 'text');
	if(count($_POST['IDlist'])>0) {
		foreach($_POST['IDlist'] as $value)	{
			$value = format_data($value, 'int');
			$query = "update ".$table_name." set output=1 where Fullkey='$value'";
			$obj_dogs->run_mysql($query);
		}
	}
	js_a_l('修改完成', 'dogs_airport.php?1=1'.$my_query_string);exit;
}elseif($_POST['action']=='unoutput_all') {	//批次修改狀態
	$my_query_string = format_data($_POST['my_query_string'], 'text');
	if(count($_POST['IDlist'])>0) {
		foreach($_POST['IDlist'] as $value)	{
			$value = format_data($value, 'int');
			$query = "update ".$table_name." set output=0 where Fullkey='$value'";
			$obj_dogs->run_mysql($query);
		}
	}
	js_a_l('修改完成', 'dogs_airport.php?1=1'.$my_query_string);exit;
...
if($search_type=='output0') {
	$where_str .= " and output_state=0";	
}elseif($search_type=='output1') {
	$where_str .= " and output_state=1";	
}
$query = "select Fullkey, file1, name, sex, years, month, weight, handler, verify, output, output_state, adopter, remark, sort, pub, create_time, edit_time from $table_name where lang='".$_SESSION[Login_System_User]['lang']."' $where_str order by sort desc";	
  ...
  <?php include("include_menu.php"); ?><!--slidebar end-->
   ...
   <div class="mainContent">
        ...
        <div class="toolbar">
		  ...
		  <input type="button" class="batchButton2" value="Batch Airport" onClick="$('#action').val('output_all');$('#list_form').submit();">
		  <input type="button" class="batchButton2" value="Batch Unairport" onClick="$('#action').val('unoutput_all');$('#list_form').submit();">
          <a href="<?=$main_str?>_airportList_01.php" target="_blank"><input type="button" value="Airport List 1"></a>
          <a href="<?=$main_str?>_airportList_02.php" target="_blank"><input type="button" value="Airport List 2"></a>
		  ...
		  <!--<div style="float:left;">End Date：</div><div id="to_date" style="width:400px;"></div>--><input type="submit" value="儲存">
          </form>
          </div>
        </div>
        ...
        <input type="hidden" name="my_query_string" id="my_query_string" value="<?=$my_query_string?>">
          <table width="100%"  cellspacing="0" cellpadding="0" border="0">
              ...
              <tr>
                ...
                <td align="center"><input type="checkbox" name="data_output_state" id="data_output_state<?=$i?>" value="1" <?php if($dogs['output_state']==1){echo 'checked';} ?> onClick="change_power(<?=$dogs['Fullkey']?>, 'output_state', this.checked)" /></td>
