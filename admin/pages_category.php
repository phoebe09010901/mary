...
if($_GET['action']=='delete') {
	...
	//檢查其下是否有類別或是資料
	$query = "select Fullkey from ".$table_name_pages."_category where prev='".$Fullkey."'";
	$obj_cate->run_mysql_list($query);
	if($obj_cate->obj_all > 0) {
		js_a_l('該類別下尚有次類別，禁止刪除', 'back');exit;	
	}
	$query = "select Fullkey from ".$table_name_pages." where category='".$Fullkey."'";
        ...
        <input type="hidden" name="action" id="action" value="">
          <table width="100%"  cellspacing="2" cellpadding="0" border="0">
              ...
              <tr>
                ...
                <td align="center"><a href="<?=($cate['lv']<News_Category_Lv_Num)?'pages_category.php?prev='.$cate['Fullkey']:'pages.php?category='.$cate['Fullkey']?>"><?=$cate['name']?></a></td>
