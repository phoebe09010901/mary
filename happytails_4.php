...
//album
$query = "select * from $table_name_album where Fullkey='$album_id'";
$album = $obj_album->run_mysql_out($query);
//photo list
$query = "select * from ".$table_name_album."_photos where album_id='".$album_id."' order by sort desc, Fullkey desc";
        ...
        <div id="qaContent">
            <div class="happy_6_2">
				...
				for($i=1; $i<=3; $i++) {
					$youtube_id = '';
                    if($album['youtube'.$i]) {
                        $youtube = explode("//", $album['youtube'.$i]);
                        $youtube = explode("/", $youtube[count($youtube)-1]);
                        $youtube_id = $youtube[count($youtube)-1];
                    }
					?><div class="movie_block"><?php
                    if($youtube_id) {
                   		?><iframe width="223" height="125" src="//www.youtube.com/embed/<?=$youtube_id?>" frameborder="0" allowfullscreen></iframe><?php
                    }elseif($album['other_video'.$i]) {
                      	echo stripslashes($album['other_video'.$i]);
                    }
					?><div style="width:100%; height:10px;"></div><?=stripslashes($album['video_title'.$i])?></div><?php
				}
				?>
				<?php
